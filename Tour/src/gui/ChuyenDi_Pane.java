...
//		ngayKetThuc.setDate(date);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (ngayKhoiHanh.getDate() != null && ngayKetThuc.getDate() != null) {
			...
			soLuongChuyenDi = cd_DAO.soLuongChuyenDi(noiKhoiHanh, min, max, ngayKH, ngayKT);

		} else
			soLuongChuyenDi = cd_DAO.soLuongChuyenDi(noiKhoiHanh, min, max);
		tongTrang = soLuongChuyenDi % 20 == 0 ? soLuongChuyenDi / 20 : (soLuongChuyenDi / 20) + 1;
		Object o = e.getSource();
		if (o.equals(noiKhoiHanhComboBox) || o.equals(giaComboBox)) {
			...
			if(table.getSelectedRow() != -1) {
				...
				}else if(trangThai.equals("Bắt đầu")) {
	...
	private void setChiTiet(String maChuyenDi) {
		...
		slideShowAnh.stop();
		slideShowAnh.start();
		moTa.setText(cd_DAO.getChuyenDi(maChuyenDi).getMoTa() + "\r\n\r\nKhởi hành:\t"
				+ sdf.format(cd_DAO.getChuyenDi(maChuyenDi).getNgayKhoiHanh()) + "\r\nKết thúc:\t"
				+ sdf.format(cd_DAO.getChuyenDi(maChuyenDi).getNgayKetThuc()) + "\r\nNơi khởi hành:\t"
				+ cd_DAO.getChuyenDi(maChuyenDi).getNoiKhoiHanh().getTinhThanh() + "\r\nGiá:\t"
				+ df.format(cd_DAO.getChuyenDi(maChuyenDi).getGiaChuyenDi()) + "\r\nSố chỗ:\t"
				+ cd_DAO.getChuyenDi(maChuyenDi).getSoChoConNhan() + "/" + cd_DAO.getChuyenDi(maChuyenDi).getSoCho());
	...
	private void themVaoBangChuyenDi() {
		...
		if (ngayKhoiHanh.getDate() != null && ngayKetThuc.getDate() != null) {
			ngayKH = Date.valueOf(sdf2.format(ngayKhoiHanh.getDate()));
			ngayKT = Date.valueOf(sdf2.format(ngayKetThuc.getDate()));
			dsChuyenDi = cd_DAO.themVaoBangChuyenDi(page - 1, noiKhoiHanh, min, max, ngayKH, ngayKT);
		} else
			dsChuyenDi = cd_DAO.themVaoBangChuyenDi(page - 1, noiKhoiHanh, min, max);
	...
	private void createDialogThemDiaDanh() {
			...
			public void actionPerformed(ActionEvent e) {
				if(tableModel.getRowCount() != 0) {
					...
					for (int i = 0; i < tableDiaDanhModel.getRowCount(); i++) {
						...
						CT_ThamQuan ct_ThamQuan = new CT_ThamQuan(DiaDanh_DAO.getDiaDanh(maDiaDanh),
								cd_DAO.getChuyenDi(maChuyenDi));
						CT_ThamQuan_DAO.insertCT_ThamQuan(ct_ThamQuan);
					}
